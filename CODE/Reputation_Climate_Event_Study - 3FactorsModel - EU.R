...
event_wdw_p <- c(35,40,45) # Event window POST
...
if(UPDATA){
 ...
 ggplot(DB, aes(x = Date, y = Value, color = ISIN, group = ISIN)) +
  ...
  save("MKT_ts", "RF_ts", "EVENTS", "DB", "FFF_ts", file = paste0(folder, "INPUT/EQUITY_DB_REPRISK.Rdata") )
  
  # rm(list= ls()[!(ls() %in% c("folder", "filein", "MKT_ts", "RF_ts", "ANA",  "EVENTS", "DB", "FFF_ts"))])
  
}else {
  tmp.env   <- new.env()
  load( paste0(folder,"INPUT/EQUITY_DB_REPRISK.Rdata" ), envir=tmp.env)
...
save.image(file = paste0(folder,  "OUTPUT/results_before_means_45days.RData"))
...
if (EXPORT){
  ...
  # RESULTS + MODEL STATS
  file <- paste0 ( fileout, "_raw/Rep_Climate_3FactorsModel_15days.xlsx") 
  write.xlsx(list( 
