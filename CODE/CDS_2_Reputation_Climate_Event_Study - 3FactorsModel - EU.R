...
event_wdw_p <- c(5) # Event window POST
...
if(UPDATA){
  ...
  # Price Index
  Xi   <- read.xlsx( paste0(folder,"DATA/EQUITY_Prices.xlsx"), sheet = "DATA_april2024", colNames = TRUE, detectDates = TRUE, na.strings = c( "NA", "#N/A", "N/D" ) )
  ...
  DB <- DB %>%
    group_by(ISIN) %>%
    mutate(rolling_variance = rollapply(Return, width = 250, FUN = var, by.column = FALSE, fill = NA, align = "right")) %>% as.data.frame()
  
  DB <- DB %>%
    group_by(ISIN) %>%
    mutate(ann_rolling_variance = rolling_variance*250) %>% as.data.frame()
  
  DB <- DB %>%
    group_by(ISIN) %>%
    mutate(delta_var = ann_rolling_variance - dplyr::lag(ann_rolling_variance,1)) %>% as.data.frame() 
  
  #DB_ts <- xts( DB[, c("Return", "delta_var")], as.Date( DB[,1], origin = "1899-12-30" ) ) # time series
  #colnames(DB_ts) <- c("Return", "delta_var")
  
  colnames(DB) <- c("Date", "ISIN", "Equity_Value", "Equity_Return", "roll_var", "ann_roll_var", "delta_var")
  
  DBPD <- merge(DBPD, DB, by = c("ISIN", "Date"))
...
DSET2 <- DBPD %>% 
  filter(!ISIN %in% as.data.frame(ISIN_toclean_s1)[,1] ) %>% 
  select(Date, ISIN, Equity_Return) %>%
  dcast(Date ~ ISIN, fun.aggregate = mean, value.var = "Equity_Return") # cast (ISIN = columns)

colnames(DSET2) <- paste("Eq_", colnames(DSET2[,-1]), sep = "")


DSET3 <- DBPD %>% 
  filter(!ISIN %in% as.data.frame(ISIN_toclean_s1)[,1] ) %>% 
  select(Date, ISIN, delta_var) %>%
  dcast(Date ~ ISIN, fun.aggregate = mean, value.var = "delta_var") # cast (ISIN = columns)

colnames(DSET3) <- paste("dv_", colnames(DSET3[,-1]), sep = "")
...
for ( k in 1:length(event_wdw_p)) {
  ...
  rr <- xts( DSET2[,2:ncol(DSET2)], t ) 
  rrr <- xts( DSET3[,2:ncol(DSET3)], t )
  ...
  r <- merge(r, rr, join='left' ) 
  r <- merge(r, rrr, join='left' ) 
  ...
  for ( j in 1:nrow(EVENTS)) {
    ...
    r_rw_temp <- r [, c(Issuer_of_event, "Mkt",riskfree, "SMB" ,  "HML", "Def",  "Term", paste0("Eq_",Issuer_of_event),paste0("dv_",Issuer_of_event))]
    ...
    if(ncol(r_rw) > 8){ # se non è stato eliminato per qualche motivo l'emittente
      ...
      X    <- cbind(ones, r_ev[, "Mkt"], r_ev[, "SMB"], r_ev[, "HML"], r_ev[, "Def"], r_ev[, "Term"],r_ev[, paste0("Eq_",Issuer_of_event)],r_ev[, paste0("dv_",Issuer_of_event)]) 
      
      # Estimate linear model at bond level in the estimation window
      mm_event   <- lm( r_est[, Issuer_of_event] - r_est[, riskfree]  ~ (r_est[, "Mkt"] - r_est[, riskfree]) +  r_est[, "SMB"] + r_est[, "HML"] +  r_est[, "Def"] + r_est[, "Term"]+ r_est[, paste0("Eq_",Issuer_of_event)] + r_est[, paste0("dv_",Issuer_of_event)]) 
      ...
      X_est  <- cbind(ones_est, r_est[, "Mkt"], r_est[, "SMB"], r_est[, "HML"], r_est[, "Def"], r_est[, "Term"], r_est[, paste0("Eq_",Issuer_of_event)], r_est[, paste0("dv_",Issuer_of_event)]) 
...
save.image(file = paste0(folder,  "OUTPUT_CDS/results_before_means_eq_dv.RData"))
...
if (EXPORT){
  ...
  # RESULTS + MODEL STATS
  file <- paste0 ( fileout, "_raw/CDS_Rep_Climate_3FactorsModel_15days.xlsx") 
  write.xlsx(list( 
