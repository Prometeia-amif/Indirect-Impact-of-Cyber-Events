...
if(UPDATA){
  ...
  EVENTS    <- read.xlsx( paste0(folder,"OUTPUT/EVENTS_SPILL_08_spearman_with_info_VAR_model_other_firms2_MKT_more_firms_FF.xlsx"), sheet = "events", colNames = TRUE, detectDates = TRUE, na.strings = c( "NA", "#N/A", "N/D" ) )
...
#save.image(file = paste0(folder,  "OUTPUT/results_before_means_CYBER_MARYLAND_2017_2024_spill_VAR_other_firms2.RData"))
save.image(file = paste0(folder,  "OUTPUT/results_before_means_CYBER_MARYLAND_2017_2024_spill_VAR_other_firms_MKT_Sept2025_more_firms_FF.RData"))

#load(paste0(folder,  "OUTPUT/results_before_means_CYBER_MARYLAND_2017_2024_spill_VAR.RData"))
load(paste0(folder,  "OUTPUT/results_before_means_CYBER_MARYLAND_2017_2024_spill_VAR_other_firms_MKT_Sept2025_more_firms_FF.RData"))
...
round(avg_per_date)
#CAR_all<-CAR_all%>%filter(Date>="2019-01-01")
...
ggplot() +
 ...
 #   path = "C:/Users/ivan.delorenzo/OneDrive - PROMETEIA SPA/Desktop/events.xlsx"
 # )

orig_counts_q2<- orig_counts_q %>% left_join(counts_q, by ="quarter")
orig_counts_q2$ratio<-orig_counts_q2$n.y/orig_counts_q2$n.x

ggplot() +
geom_line(
  data = orig_counts_q2,
  aes(x = quarter, y = ratio, color = "Indirect/Direct events", group = 1),
  size = 1.2
)+
  labs(
    title = "Indirect/Direct events",
    x = "Quarter",
    fill = NULL,   # removes fill legend title
    color = NULL   # removes color legend title
  ) +
