...
load("G:/CLIMATE RISK & ESG/PROGETTI/Reputation_risk/REPUTATION/OUTPUT/results_before_means_CYBER_MARYLAND_2017_2024.RData")
...
for (col in setdiff(names(db_regre), c("Date", "MonthYear", "Mkt", 'UA4000067300', 'UA4000166011'))) {
...
residuals_df$Date <- db_regre$Date
residuals_df$MonthYear <- db_regre$MonthYear

gXi2 <- residuals_df

gXi22 <- gXi2 %>% filter(MonthYear<="2015-01")
corrmatrix<-cor(gXi22[,1:1025], method = my_method, use = "complete.obs")
...
save.image("G:/CLIMATE RISK & ESG/PROGETTI/Reputation_risk/REPUTATION/OUTPUT/network_resid_VAR_other_firms2_MKT_3_k.RData")
...
file <- "C:/Users/ivan.delorenzo/OneDrive - PROMETEIA SPA/Desktop/Rep Risk/REPUTATION/OUTPUT/EVENTS_SPILL_08_spearman_with_info_VAR_model_other_firms2_MKT_more_firms.xlsx"
write.xlsx(list( 
  events=EVENTS_SPILL),
  file)
